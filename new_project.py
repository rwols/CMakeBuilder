...
class CmakeNewProjectCommand(sublime_plugin.WindowCommand):
    ...
    def _on_done_project_name(self, text):
        ...
        self.window.show_input_panel(
            "Type the path where you want to initialize %s: " % self.project_name,
            initial_text,
            self._on_done_project_dir,
            None,
            None,
        )
    ...
    def _on_done_project_dir(self, text):
        ...
        if os.path.exists(self.project_dir):
            sublime.error_message(
                'The directory "%s" already exists!' % self.project_dir
            )
            return
        ...
        if sublime.ok_cancel_dialog(msg):
            self.window.show_quick_panel(
                [
                    ["C", "A project suitable for C"],
                    ["C++", "A project suitable for C++"],
                    ["C and C++", "A project suitable for both C and C++"],
                ],
                self._on_done_select_project_type,
            )
    ...
    def _create_project(self):
        ...
        with open(
            os.path.join(self.project_dir, "src", "main." + self.suffix), "w"
        ) as f:
            if self.type == "C":
                f.write(CFILE)
            else:
        ...
        project_file = os.path.join(
            self.project_dir, self.project_name + ".sublime-project"
        )
        with open(project_file, "w") as f:
            f.write(PROJECTFILE)
        if sublime.ok_cancel_dialog(
            'Select the file %s in "%s" in the upcoming prompt...'
            % (self.project_name + ".sublime-project", self.project_dir)
        ):
